...
`Python` enables decoupling the data and logic, but the tradeoff is that the switch is no intuitive. Skillset will be the first gap to deal with - You first need to learn the basics of `Python` and the `pandas` "extension" (dependency) for spreadsheet operations. Next, you'll need some compute resources/tool to run your scripts (like `Jupyter Notebook`). If you have trouble installing everything, you may have no choice but to use the command line, overall setting the environment should be straight forward and has lots of documentation available. If you want to share your work (which you should if the team size is > 2), you'll also need to specify the versions of your dependencies using a configuration file to avoid version mismatches. **This is a lot to ask.**
